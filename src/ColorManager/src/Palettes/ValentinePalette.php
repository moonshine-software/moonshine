<?php

declare(strict_types=1);

namespace MoonShine\ColorManager\Palettes;

use MoonShine\Contracts\ColorManager\PaletteContract;

final class ValentinePalette implements PaletteContract
{
    public function getDescription(): string
    {
        return 'Romantic red-pink dual-tone';
    }

    public function getColors(): array
    {
        return [
            'body' => '0.98 0.02 340',
            'primary' => '0.55 0.28 355',
            'primary-text' => '0.99 0.01 340',
            'secondary' => '0.92 0.08 340',
            'secondary-text' => '0.22 0.03 355',
            'base' => [
                'text' => '0.22 0.03 355',
                'stroke' => '0.55 0.28 355 / 20%',
                'default' => '0.98 0.02 340',
                50 => '0.96 0.04 340',
                100 => '0.93 0.06 340',
                200 => '0.90 0.09 340',
                300 => '0.86 0.12 340',
                400 => '0.81 0.16 340',
                500 => '0.74 0.20 355',
                600 => '0.65 0.24 355',
                700 => '0.55 0.28 355',
                800 => '0.45 0.22 355',
                900 => '0.35 0.16 355',
            ],
            'success' => '0.64 0.22 142.49',
            'success-text' => '0.46 0.16 142.49',
            'warning' => '0.75 0.17 75.35',
            'warning-text' => '0.5 0.10 76.10',
            'error' => '0.58 0.21 26.855',
            'error-text' => '0.37 0.145 26.85',
            'info' => '0.60 0.219 257.63',
            'info-text' => '0.35 0.12 257.63',
        ];
    }

    public function getDarkColors(): array
    {
        return [
            'body' => '0.18 0.06 340',
            'primary' => '0.70 0.22 355',
            'primary-text' => '0.16 0.07 340',
            'secondary' => '0.48 0.16 340',
            'secondary-text' => '0.94 0.05 355',
            'base' => [
                'text' => '0.90 0.04 340',
                'stroke' => '0.70 0.22 355 / 20%',
                'default' => '0.22 0.07 340',
                50 => '0.24 0.07 340',
                100 => '0.29 0.08 340',
                200 => '0.34 0.10 340',
                300 => '0.41 0.12 340',
                400 => '0.49 0.14 340',
                500 => '0.57 0.17 355',
                600 => '0.64 0.20 355',
                700 => '0.70 0.22 355',
                800 => '0.78 0.18 355',
                900 => '0.85 0.14 355',
            ],
            'success' => '0.64 0.22 142.495',
            'success-text' => '0.93 0.12 144.46',
            'warning' => '0.9 0.22 92.72',
            'warning-text' => '0.99 0.072 107.64',
            'error' => '0.589 0.214 26.855',
            'error-text' => '0.71 0.24 25.96',
            'info' => '0.6 0.22 257.63',
            'info-text' => '0.88 0.065 244.38',
        ];
    }
}
