export default function jsonField(initialRows = [], fields = [], removable = true, reorderable = false, keyValue = false, onlyValue = false, objectMode = false, filterEmpty = true, creatable = true, creatableLimit = null) {
  return {
    rows: [],
    fields: fields,
    removable: removable,
    ...
    emptyValue(field) {
      ...
      if (field.type === 'select' && !field.nullable && Array.isArray(field.options) && field.options.length > 0) {
    ...
    normalizeRows(value) {
      ...
      if (this.keyValue && value && !Array.isArray(value) && typeof value === 'object') {
        ...
        return Object.entries(value).map(([key, itemValue]) => this.normalizeRow({
          [keyField.column]: key,
      ...
      if (this.onlyValue && Array.isArray(value)) {
        ...
        return value.map((itemValue) => {
          const row = itemValue && !Array.isArray(itemValue) && typeof itemValue === 'object'
            ? itemValue
            : {[valueField.column]: itemValue};
    ...
    normalizeSelectValue(value, field) {
      ...
      if (!field.nullable && (value === undefined || value === null || value === '') && Array.isArray(field.options) && field.options.length > 0) {
        return String(field.options[0].value);
      }
    ...
    normalizeJsonValue(value, field) {
      ...
      if (field.keyValue && value && !Array.isArray(value) && typeof value === 'object') {
        ...
        return Object.entries(value).map(([key, itemValue]) => this.normalizeRowForFields({
          [keyField.column]: key,
          [valueField.column]: itemValue,
        }, field.fields || []));
      }
      ...
      return (Array.isArray(value) ? value : []).map((row) => this.normalizeRowForFields(row, field.fields || []));
    },
    serialize() {
      if (this.keyValue) {
      ...
      return this.filterSerializedRows(this.rows.map((row) => {
    ...
    serializeJsonValue(value, field) {
      ...
      return this.filterSerializedRows(rows.map((row) => {
    ...
    updateDropIndex(clientY) {
      ...
      const rows = [...this.dragContainer.querySelectorAll(':scope > .json-field__item > .json-field__row[data-json-row-index]')];
    ...
    dragItemElements() {
      ...
      return [...this.dragContainer.querySelectorAll(':scope > .json-field__item > .json-field__row[data-json-row-index]:not(.json-field__row--dragging)')];
    },
