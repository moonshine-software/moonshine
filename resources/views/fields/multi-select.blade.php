<div x-data="multiSelect_{{ $element->id() }}()" x-init="loadOptions()"
     class="w-full flex flex-col items-center mx-auto">
    <select
        x-model="{{ $element->isMultiple() ? 'selectValues' : 'selectValue' }}"
        x-ref="multi_select_{{ $element->id() }}"
        {{ $element->attributes()  }}
        id="{{ $element->id() }}"
        name="{{ $element->name() }}"
        ...
        style="display: none;"
    >
        @if(!$element->isMultiple() && $element->isNullable())
    ...
    <div class="inline-block relative w-full">
        <div class="flex flex-col items-center relative">
            <div x-on:click="open" class="w-full">
                <div
                    class="p-1 flex items-center justify-between border border-gray-200 bg-white dark:bg-darkblue rounded">
                    <div class="flex flex-auto flex-wrap">
                        <template x-for="(option,index) in selectedOptions()" :key="index">
                            <div
                                class="flex justify-center items-center m-2 p-2 font-medium bg-purple rounded text-white border">
                    ...
                    <div
                        class="text-gray-300 dark:text-white w-8 py-1 pl-2 pr-1 border-l flex items-center border-gray-200">
	...
	"/>
            ...
            <div class="w-full px-4">
                <div x-show.transition.origin.top="isOpen()"
                     class="absolute shadow-lg top-100 bg-white dark:bg-darkblue z-40 w-full left-0 rounded max-h-select"
                    ...
                    <div class="flex flex-col w-full overflow-y-auto h-64">
                        <template x-for="(option,index) in options" :key="index" class="overflow-auto">
                            <div x-show="!option.hidden"
                                 class="cursor-pointer w-full border-gray-100 rounded-t border-b"
                                 @click="select(index)">
                                <div class="flex w-full items-center p-2 pl-2 border-transparent border-l-2 relative">
                                    <div class="w-full items-center flex justify-between">
                                        <div class="mx-2 leading-6 text-black dark:text-white" x-model="option"
                                             x-text="option.text"></div>
                                        <div x-show="option.selected">
                                            <svg class="text-purple w-6 h-6" fill="none" stroke="currentColor"
...
<script>
    function multiSelect_{{ $element->id() }}() {
        return {
            search: '',
            options: [],
            selectValues: [],
            ...
            loadOptions() {
                const options = this.$refs.multi_select_{{ $element->id() }}.options;
