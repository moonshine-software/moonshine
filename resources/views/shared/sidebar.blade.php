...
>
    <div class="m-6">
        <a href="{{ route(config('moonshine.route.prefix') . '.index') }}" class="flex items-center justify-start text-purple space-x-2">
            @if(config("moonshine.logo"))
                <img class="rounded-full h-10 mb-3 mx-auto" src="{{ config("moonshine.logo") }}" alt="{{ config("moonshine.title") }}">
            @else
                <svg class="fill-current h-10" viewBox="0 0 24 24">
                ...
                <span class="font-bold text-2xl">{{ config("moonshine.title") }}</span>
            @endif
        </a>
    </div>
