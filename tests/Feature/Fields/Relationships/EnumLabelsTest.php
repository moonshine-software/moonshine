...
it('renders enum labels in async relationship search', function (string $enumClass, string $raw, string $label) {
    $item = createItem(countComments: 0);
    $item->update(['name' => $raw]);
    $item->mergeCasts(['name' => $enumClass]);

    $field = BelongsTo::make('Item', 'item', resource: TestItemResource::class)->asyncSearch('name');
